...
Music.prototype = {
    appendCss : function () {
        $('head').append('<link rel="stylesheet"  href="music.css">');
        $('head').append('<link rel="stylesheet"  href="Font-Awesome-master/css/font-awesome.min.css">');
    },
    appendHtml:function () {
        var me = this ;
        var audio = new Audio();
    ...
    dialog:function ($node) {
        ...
        $node.on("mousedown",function (e) {
            var X = e.pageX - $node.offset().left; //鼠标在dialog内的x轴位置
            var Y = e.pageY - $node.offset().top; //鼠标在dialog内的y轴位置
            $node.addClass('cursor').data("inner",{innerX:X,innerY:Y});
        });
        ...
        $('body').on("mousemove",function (e) {
            if($node.data("inner")){
                $node.offset({
                    left : e.pageX-$node.data("inner").innerX,
                    top : e.pageY-$node.data("inner").innerY
                });
            }
        });
    },
    ...
    bind:function () {
        ...
        $('#jquery-music .song-volume').on('click',function (e) {
            ...
            if(!$('.song-volume').data('mute')){
                ...
                me.volumeValue = 0;
                $('.volume-bar').height(0);
                $('.song-volume>i').attr('class','fa fa-volume-off');
                $('.song-volume').data('mute',true);
            }
            else{
                me.audio.volume = me.muteVolumeValue*0.05;
                me.volumeValue = me.muteVolumeValue;
                $('.volume-bar').height(me.volumeValue*4);
                if(me.audio.volume === 0){
                    $('.song-volume>i').attr('class','fa fa-volume-off');
                }
                if(me.audio.volume < 0.5 && me.audio.volume!==0){
                    $('.song-volume>i').attr('class','fa fa-volume-down');
                }
                if(me.audio.volume >= 0.5){
                    $('.song-volume>i').attr('class','fa fa-volume-up');
                }
                $('.song-volume').data('mute',false);
            }
        });
        //关闭歌词面板
